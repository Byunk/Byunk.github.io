+++
title = "Hey! I'm Kyungho"
readingTime = false
Toc = true
+++

My name is kyungho and I am living in Seoul, Korea

{{< figure src="profile.jpg" position="center" style="border-radius: 8px; width: 35%; margin: 0 auto;" caption="When I was traveling around Istanbul, Turkey" captionPosition="center" captionStyle="color: black;">}}

## Academics

I am an undergraduate student at KAIST, majoring in computer science and aerospace engineering.

My favorite CS courses include:

-   CS330: Operating Systems and Lab
-   CS360: Introduction to Databases
-   CS341: Introduction to Computer Networks

In the AE department, I have particularly enjoyed:

-   AE250: Aerospace Dynamics
-   AE400: Aerospace System Design
-   AE405: Satellite Systems

As a Backend Tech Lead at GDSC KAIST, I am leading Board of Audit and Inspection system project. We have digitalized auditing process and system for student clubs.

## Career

[LinkedIn Profile](https://www.linkedin.com/in/kyungho-byoun/)

I currently work at [SAP](https://www.sap.com/about/what-is-sap.html) Labs Korea as a cloud engineer. My ongoing project involves developing a testing tool to facilitate database engineers in testing their new features within a cloud environment. Additionally, I am working with Kubernetes. I feel love with Kubernetes, even though I've only been studying it for two months.

From July, 2022 to June, 2023, I worked as a researcher at the [KI-ITC Augmented Reality Research Centre, KAIST](https://www.linkedin.com/company/kaistarrc/?originalSubdomain=kr). I analyzed cardiorespiratory signals from a bed pressure sensor to classify sleep stages. Also I developed a classification logic that will be embedded in a smart mattress. I utilized Matlab for signal analysis and Javascript for the classification logic.

In the second semester of 2022, I led a startup team called Treevial, consisting of 5 members. Our goal was to generate profits from forests through carbon credits to encourage afforestation by individuals and companies. We were finalists in App Startup Competition held by KAIST.

In between semesters, I worked twice as an undergraduate research intern at the [STRATEGIC AEROSPACE INITIATIVE Lab](http://sai.kaist.ac.kr/), KAIST. The main focus of my research was on routing problems, and I successfully reproduced few papers using Matlab.

## Academic Interests

-   I am an open source contributor
    -   [sql-metadata](https://github.com/macbre/sql-metadata)
-   Recently, I have found fun solving competitive programming questions. I am work in progress.
    -   [Leetcode](https://github.com/Byunk/Leetcode)
    -   [Baekjoon: Korean competitive programming question archieve](https://github.com/Byunk/Baekjoon)

## What I Love

-   I am a Ryan lover.

{{< figure src="ryan.jpg" position="center" style="border-radius: 8px; width: 35%; margin: 0 auto;">}}

-   I usually end up the week with [Spurs](https://en.wikipedia.org/wiki/Tottenham_Hotspur_F.C.). My favorite player is obviously Sonny.
-   I also love playing soccer and futsal. I was even a school representative when I was in elementary school.
-   I am a minimalist. I am participating in [project 333 challenge](https://bemorewithless.com/project-333-challenge/).
-   I want to be a passionate reader. I read books, articles, and some business reports. I am a big fan of Dale Carnegie. I often read 10K reports by businesses for fun.
-   I am obssessed with Warren Buffett. Not only his noticeble legend of investing, but also his philosophy. He have sticked to his own principle throughout his entire life. He does not invest in businesses that he does not understand, and he always shares his knowledge with the public. I am a shareholder of Berkshire, and attending its shareholder meeting is one of my bucket list items.
